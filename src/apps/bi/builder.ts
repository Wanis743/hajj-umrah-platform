 ...
 * decision.
 *
 * This is the eleventh file in `src/apps/bi`, where DMS has ten. The divergence is
 * deliberate and there is exactly one reason for it: a six-hundred-line reducer has
 * no counterpart in DMS and would push `shell.ts` past `max-lines` on its own. It is
 * also the only part of BI that is pure state, and keeping it separate is what lets
 * `shell.ts` be about wiring rather than about array splices.
 *
 * Three things live here rather than in the panel, and each is here for the same
 * reason: the compiler in `20260901120000_bi_studio_vertical_slice.sql` refuses a
 * malformed request with 22023, and a builder that lets somebody assemble one and
 ...
 * 3. `drillStepFor` decides what a clicked mark means, and refuses the three cases
 *    that look like drills and are not: a metric cell, a period cell — `bi_period` is
 *    the compiler's own column rather than a dimension of the dataset, so a filter on
 *    it would be refused as an unknown field — and a level below that the dataset has
 *    not authored yet.
 *
 ...
 * it.
 *
 * Ported from `src/components/admin/bi/biBuilderState.ts`. Two things changed on the
 * way in. The row limits come from `BI_ROW_LIMIT_MIN/MAX/DEFAULT` rather than a
 * private copy of the same three numbers, because two constants for one server-side
 * clamp drift. And `BiQueryRequest` is imported from `./actions`, which owns the
 * request shape now that the command layer is the only thing that sends one; the
 * import is type-only, so it erases and this file stays free of React and of the
 * kernel.
 */
import {
  BI_OPERATOR_ARITY, BI_PERIOD_KEY,
  BI_ROW_LIMIT_DEFAULT, BI_ROW_LIMIT_MAX, BI_ROW_LIMIT_MIN,
} from './types';
import type {
  BiChartType, BiColumn, BiDataType, BiDimension, BiFilter, BiFilterOperator,
  BiMetric, BiScalar, BiTimeGrain,
} from './types';
import type { BiQueryRequest } from './actions';
import { CHART_FAMILY, CHART_SHAPE } from './model';
...
export interface BuilderState {
   ...
   *  compiler adds the period column itself when a grain is set, and the reducer
   *  below refuses to put it here even if a caller asks. */
  ...
  readonly trail: readonly BiDrillStep[];
}
 ...
 *
 *  The dimension shelf also refuses `bi_period`. The comment on `dimensions` has said
 *  so since the builder was written, and the rule was enforced by nobody: the palette
 *  simply does not offer the key, which is a convention rather than an invariant. The
 *  period column arrives by setting a grain, so a caller that put it on the shelf
 *  itself would produce a request the compiler refuses as an unknown field. */
function reduceShelf(state: BuilderState, action: ShelfAction): BuilderState {
  if (action.type === 'ADD_FIELD') {
    if (action.shelf === 'DIMENSION' && action.key === BI_PERIOD_KEY) return state;
...
export function clampLimit(value: number): number {
  if (!Number.isFinite(value)) return BI_ROW_LIMIT_DEFAULT;
  return Math.min(BI_ROW_LIMIT_MAX, Math.max(BI_ROW_LIMIT_MIN, Math.trunc(value)));
}
...
/** Whether a filter carries a value at all. Narrowing rather than returning a boolean
 *  is what lets `retypeFilter` move the single value into a list without a cast:
 *  `BiScalar` includes null, and a null is exactly what must not travel into
 *  `values`. */
const isFilled = (value: BiScalar | undefined): value is Exclude<BiScalar, null> =>
...
export function retypeFilter(filter: BiFilter, op: BiFilterOperator): BiFilter {
  ...
  if (arity === 'many') {
    const values = filter.values ?? (isFilled(filter.value) ? [filter.value] : []);
 ...
 * date still produced a valid result, and the frame names what is missing under the
 * chart it could not draw. A request the compiler will refuse is a different matter —
 ...
 *
 * Temporal in the same two ways the result is: a time grain, which makes the compiler
 * emit a date-typed period column, or a selected dimension that is a date in its own
 * right. Testing the grain alone would be the shorter check and the wrong one — it
 * would print "needs a date" over a schedule grouped on a `departure_date` dimension
 * that the renderer is perfectly happy with, and a readiness note that fires when the
 * chart does draw teaches the reader to stop reading them.
 */
const hasTemporal = (
  state: BuilderState, dimensions: readonly BiDimension[],
): boolean => state.timeGrain !== null
  || state.dimensions.some((key) => {
    const type = dimensions.find((dimension) => dimension.key === key)?.dataType;
...
export function readiness(
  ...
  // family asks separately — the same extra question the result asks of the chart it
  // could not draw, put to the request instead so a Gantt says what it needs before it
  // is run.
 ...
 *
 * `datasetDimensions` is the guard on the level below: `drillToKey` may name a
