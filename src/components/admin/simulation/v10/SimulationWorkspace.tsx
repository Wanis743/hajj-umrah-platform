    ...
    <div className="flex flex-col h-full bg-transparent text-white/90">
      <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/[0.04]/40 backdrop-blur-md">
        <div>
          ...
          <p className="text-sm text-white/55">Monte Carlo and sensitivity analysis</p>
        </div>
      ...
      <div className="flex-1 overflow-auto p-6 flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-1/3 space-y-6">
          <div className="bg-white/[0.04]/40 border border-white/10 rounded-xl p-5">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-white/55 mb-1.5">Simulation Type</label>
                <select className="w-full bg-transparent border border-white/10 rounded-lg p-2.5 text-sm focus:outline-none focus:border-indigo-500/50">
              ...
              <div>
                <label className="block text-xs font-medium text-white/55 mb-1.5">Iterations</label>
                <input 
                  type="number" 
                  value={monteCarloRuns}
                  onChange={(e) => setMonteCarloRuns(parseInt(e.target.value) || 1000)}
                  className="w-full bg-transparent border border-white/10 rounded-lg p-2.5 text-sm focus:outline-none focus:border-indigo-500/50"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-white/55 mb-1.5">Target Model</label>
                <select className="w-full bg-transparent border border-white/10 rounded-lg p-2.5 text-sm focus:outline-none focus:border-indigo-500/50">
          ...
          <div className="bg-white/[0.04]/40 border border-white/10 rounded-xl p-5">
            <h3 className="font-medium text-white/90 mb-4 flex items-center gap-2">
              <Activity className="w-4 h-4 text-white/55" />
              Recent Runs
            </h3>
            ...
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-xs text-white/55">Loading jobs...</p>
              </div>
            ) : error ? (
              ...
              <div className="space-y-3">
                {runs.map(run => (
                  <div key={run.id} className="p-3 bg-transparent border border-white/5 rounded-lg flex items-center justify-between group hover:border-white/10 transition-colors cursor-pointer">
                    <div>
                      <p className="text-sm font-medium text-white/80">{run.name}</p>
                      <p className="text-xs text-white/40">{run.date}</p>
                    </div>
                    {run.status === 'running' ? (
                ...
                {runs.length === 0 && (
                  <p className="text-xs text-white/40 italic text-center py-4">No recent simulation runs</p>
                )}
              </div>
            )}
          </div>
        ...
        {/* Right Preview Panel */}
        <div className="flex-1 bg-white/[0.04]/40 border border-white/10 rounded-xl flex flex-col items-center justify-center relative overflow-hidden">
          <div className="absolute top-4 left-4 flex items-center gap-2 text-white/55 text-sm">
            <Info className="w-4 h-4" />
            <span>Select a completed run to view probability distributions</span>
          </div>
          <div className="text-center p-8">
            <div className="w-24 h-24 rounded-full bg-white/[0.07] mx-auto flex items-center justify-center mb-6">
              <BarChart2 className="w-10 h-10 text-white/35" />
            </div>
            <h3 className="text-xl font-medium text-white/80 mb-2">No Results Selected</h3>
