  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-transparent text-white/90">
        ...
        <p className="text-white/55">Loading analysis workspace...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-transparent text-white/90">
      <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/[0.04]/40 backdrop-blur-md">
        <div>
          ...
          <p className="text-sm text-white/55">Semantic layer metrics and KPIs</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 bg-white/[0.04]/50 border border-white/10 rounded-lg text-sm hover:bg-white/[0.08] backdrop-blur-lg rounded-xl border border-white/10 transition-colors">
          ...
          <button 
            ...
            className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-indigo-500/20 text-indigo-400' : 'hover:bg-white/[0.08] backdrop-blur-lg rounded-xl border border-white/10 text-white/55'}`}
          >
            <Layout className="w-4 h-4" />
          </button>
          <button 
            onClick={() => setViewMode('chart')}
            className={`p-2 rounded-lg transition-colors ${viewMode === 'chart' ? 'bg-indigo-500/20 text-indigo-400' : 'hover:bg-white/[0.08] backdrop-blur-lg rounded-xl border border-white/10 text-white/55'}`}
          >
      ...
      <div className="p-6 flex-1 overflow-auto">
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {metrics.map((metric, i) => (
              <motion.div
                ...
                className="p-5 bg-white/[0.04]/40 border border-white/5 rounded-xl hover:border-white/10 transition-colors"
              >
                <div className="flex justify-between items-start mb-4">
                  <span className="text-white/55 text-sm">{metric.label}</span>
          ...
          <div className="h-full flex items-center justify-center border border-dashed border-white/10 rounded-xl bg-white/[0.04]/20">
            <div className="text-center">
              <BarChart2 className="w-12 h-12 text-white/40 mx-auto mb-4 opacity-50" />
              <h3 className="text-lg font-medium text-white/80 mb-1">Chart View Selected</h3>
