/**
 * Presentational atoms for the DMS screens. Components only -- the formatters,
 * tone tokens and read hook live in ./dmsFormat, because a .tsx file that exports
...
import {
  DASH, REVIEW_TONE, TONE_CLASS, shortHash, type Tone,
} from './dmsFormat';
import type { DmsReviewStatus } from '@/types/dms';
 ...
 *  "Document DOC-2026-0007 approved" without borrowing the danger colours. */
...
/** A SHA-256 digest, truncated, with the full value available to copy. The whole
 *  point of recording a checksum is that somebody can compare it, so it has to be
 *  gettable out of the screen. */
export function ChecksumChip({ hash, label }: { hash: string | null; label?: string }) {
  const [copied, setCopied] = useState(false);
  if (!hash) return <span className="text-[12px] text-[var(--text-muted)]">{DASH}</span>;
  const copy = () => {
  ...
  return (
    <button
      ...
      onClick={copy}
      title={hash}
      aria-label={`${label ?? 'SHA-256'} ${hash}`}
      className="inline-flex items-center gap-1 rounded border border-[var(--border)] bg-[var(--bg-subtle)] px-1.5 py-0.5 font-mono text-[11px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
    >
      {copied ? <Check className="h-3 w-3 text-[var(--success)]" /> : <Copy className="h-3 w-3" />}
      <span className="tabular">{shortHash(hash)}</span>
    </button>
  );
}

/** SEALED-and-matching vs SEALED-and-drifted. Null means "not sealed", which is
 *  neither: there is nothing to match yet. */
export function SealBadge({ matches, label }: { matches: boolean | null; label: (ok: boolean) => string }) {
  if (matches === null) return null;
  return matches ? (
    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[var(--success)]">
      <ShieldCheck className="h-3.5 w-3.5" /> {label(true)}
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[var(--danger)]">
      <ShieldAlert className="h-3.5 w-3.5" /> {label(false)}
    </span>
  );
}

/**
 * The review path as a row of steps, with the current one filled. Draws the happy
 * path only -- REJECTED, EXPIRED and SUPERSEDED are not stops on the way to
 * approval, so when the document is in one of those the stepper says so instead of
 * pretending it is mid-journey.
 */
const STEPS: readonly DmsReviewStatus[] = ['DRAFT', 'PENDING_REVIEW', 'UNDER_REVIEW', 'APPROVED'];

export function ReviewStepper({ status, labels }: {
  status: DmsReviewStatus;
  labels: Record<DmsReviewStatus, string>;
}) {
  const index = STEPS.indexOf(status);
  if (index === -1) {
    return (
      <div className="flex items-center gap-2">
        <Pill tone={REVIEW_TONE[status]}>{labels[status]}</Pill>
      </div>
    );
  }
  return (
    <ol className="flex flex-wrap items-center gap-1.5" aria-label={labels[status]}>
      {STEPS.map((step, i) => (
          ...
          <span
            ...
            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${
              i < index
                ? TONE_CLASS.good
                : i === index
                  ? TONE_CLASS[REVIEW_TONE[step]]
                  : 'bg-[var(--bg-hover)] text-[var(--text-muted)] opacity-60'
            }`}
          >
            {labels[step]}
          </span>
          {i < STEPS.length - 1 && (
            <span aria-hidden="true" className="h-px w-3 bg-[var(--border)] sm:w-5" />
          )}
        </li>
      ))}
    </ol>
