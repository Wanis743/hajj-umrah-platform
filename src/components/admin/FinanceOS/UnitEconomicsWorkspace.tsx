...
const readinessColor = (score: number) => (score > 80 ? '#34d399' : '#fbbf24');

/** Mini progress ring for a group's readiness score (0–100). */
...
/** Row in the operational-group browser list. */
function GroupRow({
  group,
  selected,
  onSelect,
    ...
    >
      <div className="flex justify-between items-start mb-1">
        ...
        <span className="flex items-center gap-2">
          <ReadinessRing score={group.readiness_score} />
          <span className={`text-xs tabular-nums ${
            group.readiness_score > 80 ? 'text-emerald-400' : 'text-amber-400'
          }`}>
      ...
      <div className="text-xs text-slate-400 flex items-center gap-3">
        <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {group.capacity} Pax</span>
        <span className="flex items-center gap-1"><FileText className="w-3 h-3" /> {group.status}</span>
      </div>
    </button>
  );
}

/** Left-hand group browser column. */
function GroupBrowser({
  ...
  onSelect: (group: OperationalGroup) => void;
}) {
  return (
...
/** Donut of revenue decomposition into direct costs vs net margin (ledger actuals, DZD). */
function RevenueComposition({ economics }: { economics: GroupEconomics }) {
  const slices = [
    {
      label: 'Direct Costs',
      value: economics.total_cost_dzd,
      color: '#fb7185',
    },
    { label: 'Net Margin', value: economics.margin_dzd, color: '#34d399' },
  ];

  if (!(economics.total_revenue_dzd > 0 && economics.margin_dzd > 0)) {
    // Honest state: a cost/margin split is only meaningful with a positive margin.
    return (
      <p className="text-xs text-slate-500">
        Revenue composition appears once this group posts a positive net margin
        with booked revenue and costs.
      </p>
    );
  }
          ...
          {economics && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="bg-blue-900/20 border border-blue-500/30 rounded-xl p-5 flex items-center gap-4">
                  <RingChart
                    baseInnerRadius={30}
                    className="shrink-0"
                    data={[
                      {
                        label: 'Margin',
                        value: Math.min(Math.max(economics.margin_percentage, 0), 100),
                        maxValue: 100,
                        color: '#60a5fa',
                      },
                    ]}
              ...
              {/* Revenue decomposition from the profitability RPC (ledger actuals).
                  A period-bucketed trajectory remains hidden until such a dataset exists (§4). */}
              <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-6">
                <h3 className="text-sm font-medium text-slate-300 mb-4">Revenue Composition (Actuals, DZD)</h3>
                <RevenueComposition economics={economics} />
              </div>
            </div>
          )}
        </div>
...
/** Right-hand inspector. §4 ZERO FAKE BUSINESS DATA: the previous hard-coded
    receivables/liabilities figures were removed. These panels render only values
    sourced from the profitability RPC once an AR/AP feed is wired. */
