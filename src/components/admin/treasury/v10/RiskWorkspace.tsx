  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-white/90">
        ...
        <p className="text-white/55">Loading risk workspace...</p>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6 text-white/90">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/55 mt-1">Enterprise risk assessment and mitigation tracking</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="p-4 bg-white/[0.04]/50 border border-white/5 rounded-xl">
          <div className="text-sm text-white/55 mb-1">Total Value at Risk (VaR)</div>
        ...
        <div className="p-4 bg-white/[0.04]/50 border border-white/5 rounded-xl">
          <div className="text-sm text-white/55 mb-1">Active High Risks</div>
      ...
      <div className="bg-white/[0.04]/40 border border-white/10 rounded-xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-white/55 uppercase bg-white/[0.04]/50">
              <tr>
            ...
            <tbody className="divide-y divide-white/5">
              {events.map((event) => (
                <tr key={event.id} className="hover:bg-white/5 transition-colors cursor-pointer group">
                  <td className="px-6 py-4 font-mono text-white/80">{event.id.substring(0, 8)}</td>
                  <td className="px-6 py-4 text-white/55">{event.category}</td>
                  <td className="px-6 py-4 font-medium text-white/90">{event.name}</td>
                  ...
                  <td className="px-6 py-4 text-white/80">{event.impact}</td>
