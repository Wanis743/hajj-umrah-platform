...
export function ChartStatFlow({
  ...
  return (
    <>
      {icon ? (
        <div className="mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-[var(--chart-background)]">
          {icon}
        </div>
      ) : null}
      <span
        className={cn("tabular-nums text-[var(--chart-foreground)]", valueClassName)}
      >
        {numberFlowReady ? (
          <NumberFlow
      ...
      <span className={cn("mt-0.5 text-[var(--chart-label)]", labelClassName)}>
        {label}
      </span>
    </>
  );
