...
begin
 ...
 select jsonb_build_object(
  ...
  -- The 'executive' object above used to end on a comma with its jsonb_build_object
  -- left open, so 'operations' through 'upcoming' were parsed as further arguments to
  -- it and the outer builder never closed: one unbalanced paren, which plpgsql reports
  -- at the far end of the body as `unexpected end of function definition at end of
  -- input` (42601). The line also carried a second copy of 'new_pilgrims',
  -- 'at_risk_receivables_dzd' and 'at_risk_receivables_sar' -- same values as the
  -- copies earlier in the same call -- which is what an append that swallowed the
  -- closing paren leaves behind. jsonb keeps the last of a duplicate key and the
  -- values were identical, so dropping them changes no output.
  --
  -- Nothing caught it because this file was recorded in the migration ledger as
  -- applied without ever being executed: Postgres had never parsed this body.
