create or replace function public.transition_pilgrim_state(p_pilgrim_id uuid,p_to_status text) returns jsonb language plpgsql security definer set search_path=public,pg_catalog as $$
...
create or replace function public.transition_visa_status(p_pilgrim_id uuid,p_to_status text) returns jsonb language plpgsql security definer set search_path=public,pg_catalog as $$
declare p public.pilgrims%rowtype; allowed boolean:=false;
begin
