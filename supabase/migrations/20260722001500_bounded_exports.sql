        ...
        WHEN 'external_operations' THEN
            SELECT COALESCE(jsonb_agg(
                jsonb_build_object(
                    ...
                    'evidence_status', COALESCE(evidence_status, 'N/A'),
                    'sla_hours', sla_hours
                ) ORDER BY created_at
            ), '[]'::jsonb)
            INTO v_result
            FROM (
                SELECT eo.*, (SELECT status FROM external_operation_evidence e WHERE e.operation_id = eo.id LIMIT 1) as evidence_status 
